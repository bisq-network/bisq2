...
public final class NetworkId implements Proto {
    ...
    private final Map<Type, Address> addressByNetworkType = new HashMap<>();

    public NetworkId(Map<Type, Address> addressByNetworkType, PubKey pubKey, String nodeId) {
        this.pubKey = pubKey;
        this.nodeId = nodeId;
    ...
    public static final class AddressTransportTypeTuple implements Proto {
        ...
        public AddressTransportTypeTuple(Type transportType, Address address) {
            this.transportType = transportType;
            this.address = address;
        }

        public static String addressByNetworkTypeToString(Map<Type, Address> addressByNetworkType) {
        ...
        public static Map<Type, Address> setToAddressesByTypeMap(String addressAsString) {
            return Stream.of(addressAsString.split("\\|"))
                    .map(e -> List.of(e.split("#")))
                    .collect(Collectors.toMap(tokens -> Type.valueOf(tokens.get(0)),
        ...
        public static AddressTransportTypeTuple fromProto(bisq.network.protobuf.AddressTransportTypeTuple proto) {
            Type transportType = ProtobufUtils.enumFromProto(Type.class, proto.getTransportType());
            return new AddressTransportTypeTuple(transportType, Address.fromProto(proto.getAddress()));
        }

        public static List<bisq.network.protobuf.AddressTransportTypeTuple> mapToProtoList(Map<Type, Address> addressByNetworkType) {
            return addressByNetworkType.entrySet().stream()
                    .sorted(Map.Entry.comparingByKey())
        ...
        public static Map<Type, Address> protoListToMap(List<bisq.network.protobuf.AddressTransportTypeTuple> addressNetworkTypeTupleList) {

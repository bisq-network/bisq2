...
public class EducationView extends TabView<EducationModel, EducationController>  {
    ...
    private VBox overviewBox = new VBox();
    ...
    public EducationView(EducationModel model, EducationController controller) {
        ...
        overviewBox.setSpacing(MARGIN);
        scrollPane.setContent(overviewBox);

        addTab(Res.get("academy.overview"), NavigationTarget.BISQ_ACADEMY);
        addTab(Res.get("academy.bisq"), NavigationTarget.BISQ_ACADEMY);
        addTab(Res.get("academy.bitcoin"), NavigationTarget.BISQ_ACADEMY);
        addTab(Res.get("academy.security"), NavigationTarget.BISQ_ACADEMY);
        addTab(Res.get("academy.privacy"), NavigationTarget.BISQ_ACADEMY);
        addTab(Res.get("academy.wallets"), NavigationTarget.BISQ_ACADEMY);
        addTab(Res.get("academy.openSource"), NavigationTarget.BISQ_ACADEMY);
    ...
    private void addHeaderBox() {
        ...
        overviewBox.getChildren().add(box);
        subscriptions.add(EasyBind.subscribe(overviewBox.widthProperty(), w -> {
            double right = overviewBox.getPadding().getRight();
    ...
    private VBox getWidgetBox(String headline, String content, String buttonLabel) {
        ...
        box.setPadding(new Insets(MARGIN - 16, 0, MARGIN - 6, MARGIN));
        subscriptions.add(EasyBind.subscribe(overviewBox.widthProperty(), w -> {
            double value = (w.doubleValue() - overviewBox.getPadding().getRight() - MARGIN + SCROLLBAR_WIDTH) / 2;
            double wrappingWidth = value - box.getPadding().getLeft() - box.getPadding().getRight();
            headlineLabel.setWrappingWidth(wrappingWidth - MARGIN);
            contentLabel.setWrappingWidth(wrappingWidth - MARGIN);
            box.setPrefWidth(value);

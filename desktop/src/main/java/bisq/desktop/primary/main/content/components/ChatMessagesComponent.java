...
public class ChatMessagesComponent {
    ...
    private static class Controller implements bisq.desktop.common.view.Controller {
        ...
        private void twoPartyPeerLeft(TwoPartyPrivateChatChannel chatChannel) {
            UIThread.run(() -> {
                boolean peerLeft = chatChannel.getChatMessages().stream()
                        .anyMatch(m -> m.getChatMessageType() == ChatMessageType.LEAVE);
...
//                    service -> {
//                        if (!(service instanceof TwoPartyPrivateChatChannelService))
//                            return;
//                        ((TwoPartyPrivateChatChannelService) service).leaveChannel((TwoPartyPrivateChatChannel) chatChannel);
...
//            );
            ...
            chatService.findChatChannelService(chatChannel).ifPresent(
                    service -> {
                        if (!(service instanceof PrivateChatChannelService))
                            return;
                        ((PrivateChatChannelService) service).leaveChannel((PrivateChatChannel) chatChannel);
                        chatService.getChatChannelSelectionServices().get(model.getChatChannelDomain()).maybeSelectFirstChannel();
                    }
            );
            ...
            // Example 3
//            chatService.findChatChannelService(chatChannel).ifPresent(
//                    service -> service.leaveChannel(chatChannel);
        }

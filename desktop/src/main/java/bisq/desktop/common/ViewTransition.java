...
public class ViewTransition {
    ...
    public ViewTransition(@Nullable Region oldViewRoot,
        ...
        fadeinDuration = oldViewRoot == null ? defaultDuration : defaultDuration / 2;

        newView.getRoot().setOpacity(0);
        if (oldViewRoot == null) {
            fadeIn();
        } else {
            oldViewX = oldViewRoot.getTranslateX();
            scheduler = UIScheduler.run(this::fadeIn).after(fadeinDuration);

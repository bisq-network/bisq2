...
import bisq.satoshisquareapp.primary.SatoshiSquarePrimaryStageController;
...
@Slf4j
public class SatoshiSquareJavaFxExecutable extends Executable<DefaultApplicationService> {
    @LateInit
    private SatoshiSquarePrimaryStageController primaryStageController;

    public SatoshiSquareJavaFxExecutable(String[] args) {
        super(args);
    }
    ...
    protected void launchApplication(String[] args) {
        new Thread(() -> {
            ...
            Application.launch(SatoshiSquareJavaFXApplication.class, args); // blocks until app is closed
        }).start();

        SatoshiSquareJavaFXApplication.onApplicationLaunched
                .whenComplete((applicationData, throwable) -> {
                    if (throwable == null) {
                        try {
                            log.info("Java FX Application launched");
                            primaryStageController = new SatoshiSquarePrimaryStageController(applicationService,
                                    applicationData,

...
public class AmountSelection extends LifecycleScope {
    ...
    private void applyPotentialTradeAmountLimits(TradeAmountRange potentialTradeAmountLimits) {
        ...
        // The slider values are fractions of the input amount range, so they change when the range
        // changes even if the amounts stay the same (e.g. a payment method switch shrinks the max).
        // The amount observers do not fire then, so the slider values are recomputed here.
    ...
    public void onSetMinTradeAmountFromInputAmount(Monetary inputAmount) {
        synchronized (draftLock) {
            ...
            if (areLimitsAvailable() && market != null && priceQuote != null) {
                TradeAmount tradeAmount = TradeAmountConversion.toTradeAmount(market, priceQuote, inputAmount);
                applyMinAmountFromInput(tradeAmount);
            }
        }
    }
    ...
    public void onSetMaxTradeAmountFromInputAmount(Monetary inputAmount) {
        synchronized (draftLock) {
            ...
            if (areLimitsAvailable() && market != null && priceQuote != null) {
                TradeAmount tradeAmount = TradeAmountConversion.toTradeAmount(market, priceQuote, inputAmount);
                applyMaxAmountFromInput(tradeAmount);
            }
        }
    }
    ...
    public void onSetMinTradeAmountFromSliderValue(double sliderValue) {
        synchronized (draftLock) {
            ...
            if (areLimitsAvailable() && minTradeAmount != null && market != null && priceQuote != null) {
                ...
                // The slider value is re-derived from the amount, which the conversion may have clamped.
                applyMinAmountFromInput(newTradeAmount);
            }
        }
    }
    ...
    public void onSetMaxTradeAmountFromSliderValue(double sliderValue) {
        synchronized (draftLock) {
            ...
            if (areLimitsAvailable() && maxTradeAmount != null && market != null && priceQuote != null) {
                ...
                // The slider value is re-derived from the amount, which the conversion may have clamped.
                applyMaxAmountFromInput(newTradeAmount);
            }
        }
    }
    ...
    // Reconcile path: both endpoints are clamped against the same limits, which preserves
    // their order, so no ordering rule runs here - the endpoints are refreshed one after the
    // other and comparing a refreshed one with a stale one would collapse a valid range.
    private void applyMinAmountAndSliderValue(TradeAmount minTradeAmount) {
        setMinTradeAmountAndSliderValue(amountLimits.clamp(minTradeAmount));
    }

    private void applyMaxAmountAndSliderValue(TradeAmount maxTradeAmount) {
        setMaxTradeAmountAndSliderValue(amountLimits.clamp(maxTradeAmount));
    }

    // Input path: RangeAmountSpec requires max >= min, so an endpoint the user moves past the
    // other drags it along instead of leaving an inverted range behind.
    private void applyMinAmountFromInput(TradeAmount minTradeAmount) {
        ...
        if (maxTradeAmount != null && isAbove(minTradeAmount, maxTradeAmount)) {
            setMaxTradeAmountAndSliderValue(minTradeAmount);
        }
    }

    private void applyMaxAmountFromInput(TradeAmount maxTradeAmount) {
        ...
        if (minTradeAmount != null && isAbove(minTradeAmount, maxTradeAmount)) {
            setMinTradeAmountAndSliderValue(maxTradeAmount);
        }
    }

    private void setMinTradeAmountAndSliderValue(TradeAmount minTradeAmount) {
        model.setMinTradeAmount(minTradeAmount);
        model.setMinAmountSliderValue(toSliderValueFromTradeAmount(minTradeAmount));
    }

    private void setMaxTradeAmountAndSliderValue(TradeAmount maxTradeAmount) {
        model.setMaxTradeAmount(maxTradeAmount);
        model.setMaxAmountSliderValue(toSliderValueFromTradeAmount(maxTradeAmount));
    }

    private static boolean isAbove(TradeAmount left, TradeAmount right) {
        return left.getQuoteSideAmount().getValue() > right.getQuoteSideAmount().getValue()
                || left.getBaseSideAmount().getValue() > right.getBaseSideAmount().getValue();
    }

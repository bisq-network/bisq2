...
public class CompletableFutureUtilsTest {
    ...
    public void testAllOfWithType() {
        CompletableFuture<Integer> future_1 = createIntegerFuture(200, 1);
        CompletableFuture<Integer> future_2 = createIntegerFuture(100, 2);
        future_1.whenComplete((result, throwable) -> {
            log.info("future_1 completed {}", result, throwable);

        });
        future_2.whenComplete((result, throwable) -> {
            log.info("future_2 completed {}", result, throwable);
    ...
    public void testAllWithCancel() {
        CompletableFuture<Integer> future_1 = createIntegerFuture(100, 1);
        ...
        future_3.whenComplete((result, throwable) -> {
            assertNotNull(throwable);
        });

        Exception exception = assertThrows(ExecutionException.class, () -> {
            CompletableFutureUtils.allOf(future_1, future_2).get();
        });

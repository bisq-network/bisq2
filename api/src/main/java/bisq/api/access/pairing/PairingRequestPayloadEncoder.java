...
public class PairingRequestPayloadEncoder {
    ...
    public static byte[] encode(PairingRequestPayload pairingRequestPayload) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ...
            checkArgument(pairingRequestPayload.getDeviceName().length() >= 4, "DeviceName must have a t least 4 chars");
            checkArgument(pairingRequestPayload.getDeviceName().length() <= 32, "DeviceName must not be longer as 32 chars");

...
public final class MuSigArbitrationStateChangeMessage implements MailboxMessage, ExternalNetworkMessage {
    ...
    public void verify() {
        ...
        if (arbitrationCaseState == ArbitrationCaseState.CLOSED && muSigArbitrationResult.isEmpty()) {
            throw new IllegalArgumentException("Closed arbitration case state must contain MuSigArbitrationResult.");
        }
        if (muSigArbitrationResult.isPresent() && arbitrationResultSignature.isEmpty()) {
            throw new IllegalArgumentException("MuSig arbitration result must contain arbitrationResultSignature.");
        }
        if (arbitrationResultSignature.isPresent() && muSigArbitrationResult.isEmpty()) {
            throw new IllegalArgumentException("MuSig arbitration result signature requires muSigArbitrationResult.");
        }

...
public final class ModeratorStore implements PersistableStore<ModeratorStore> {
    ...
    @Override
    public bisq.support.protobuf.ModeratorStore.Builder getBuilder(boolean ignoreAnnotation) {
        return bisq.support.protobuf.ModeratorStore.newBuilder()
                .addAllReportToModeratorMessages(reportToModeratorMessages.stream()
                        .map(e -> e.toReportToModeratorMessageProto(ignoreAnnotation))
                        .collect(Collectors.toList()));
    }

    @Override
    public bisq.support.protobuf.ModeratorStore toProto(boolean ignoreAnnotation) {

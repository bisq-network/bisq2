...
public final class HalCashAccount extends Account<HalCashAccountPayload, FiatPaymentMethod> {

    private static final FiatPaymentMethod PAYMENT_METHOD = FiatPaymentMethod.fromPaymentRail(FiatPaymentRail.HAL_CASH);

    public HalCashAccount(String accountName, HalCashAccountPayload accountPayload) {
    ...
    public bisq.account.protobuf.Account.Builder getBuilder(boolean serializeForHash) {
        return getAccountBuilder(serializeForHash)
                .setHalCashAccount(toHalCashAccountProto(serializeForHash));
    }

...
public final class SbpAccount extends Account<SbpAccountPayload, FiatPaymentMethod> {

    private static final FiatPaymentMethod PAYMENT_METHOD = FiatPaymentMethod.fromPaymentRail(FiatPaymentRail.SBP);

    public SbpAccount(long creationDate, String accountName, SbpAccountPayload accountPayload) {
    ...
    public bisq.account.protobuf.Account.Builder getBuilder(boolean serializeForHash) {
        return getAccountBuilder(serializeForHash)
                .setSbpAccount(toSbpAccountProto(serializeForHash));
    }

    public static SbpAccount fromProto(bisq.account.protobuf.Account proto) {

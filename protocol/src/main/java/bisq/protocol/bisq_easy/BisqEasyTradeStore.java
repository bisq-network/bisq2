...
public final class BisqEasyTradeStore implements PersistableStore<BisqEasyTradeStore> {
    ...
    public static BisqEasyTradeStore fromProto(bisq.protocol.protobuf.BisqEasyTradeStore proto) {
        return new BisqEasyTradeStore(proto.getTradeByIdMap().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey,
                        e -> BisqEasyTrade.fromProto(e.getValue()))));
    }
    ...
    public void add(BisqEasyTrade<?, ?> bisqEasyTrade) {
        String tradeId = bisqEasyTrade.getTradeId();
        if (!tradeById.containsKey(tradeId)) {
            tradeById.put(tradeId, bisqEasyTrade);
        }
    }

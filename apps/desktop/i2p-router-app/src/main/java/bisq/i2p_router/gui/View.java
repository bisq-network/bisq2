...
import bisq.i2p_router.gui.components.SpinnerAnimation;
...
public class View extends StackPane {
    private static final double SPINNER_SIZE = 67 * 0.85; // 67 orig size

    private final Controller controller;
    private final Model model;
    private final VBox errorBox;
    private final SpinnerAnimation spinner;
    private final Label headline, routerState, errorHeadline, errorDetails, duration, routerStateDetails, routerSetupDetails;
    private final TextArea errorMessage;
    private final Button button;
    private final ProgressBar progressBar;
    private final VBox contentBox, startupBox;
    private final HBox routerStateAndDurationBox;
    ...
    public View(Controller controller, Model model) {
        ...
        logo.setId("bisq-logo");
        logo.setFitWidth(SPINNER_SIZE);
        logo.setFitHeight(SPINNER_SIZE);

        ImageView i2pLogo = new ImageView();
        i2pLogo.setId("connected");
        i2pLogo.setFitWidth(SPINNER_SIZE);
        i2pLogo.setFitHeight(SPINNER_SIZE);

        spinner = new SpinnerAnimation(SPINNER_SIZE);
        spinner.setAlignment(Pos.CENTER);

        HBox logoBox = new HBox(5, logo, i2pLogo);
        ...
        routerStateAndDurationBox = new HBox(12.5, routerState, getSeparator(), duration);
        ...
        startupBox = new VBox(12.5, routerStateAndDurationBox, progressBar, routerStateDetails);
        startupBox.setAlignment(Pos.CENTER);

        Label info = new Label(Res.get("info"));
        ...
        errorDetails.getStyleClass().add("error");

        errorMessage = new TextArea();
        errorMessage.setContextMenu(new ContextMenu());
        errorMessage.setEditable(false);
        errorMessage.setWrapText(true);
        errorMessage.getStyleClass().addAll("code-block");
        ...
        VBox.setMargin(spinner, new Insets(20, 0, 10, 0));
        VBox.setMargin(button, new Insets(20, 0, 10, 0));
        contentBox = new VBox(10, logoBox, headline, info, startupBox, button, errorBox, getVSpacer(), routerSetupDetails);
            ...
            if (state != null) {
                ...
                switch (state) {
                    ...
                    case RUNNING_OK -> {
                        ...
                        duration.getStyleClass().add("highlight");
                        spinner.setI2PIconId("connected");
                    }
                    case STOPPING -> {
                    ...
                    case RUNNING_FIREWALLED, RUNNING_DISCONNECTED, STOPPED, FAILED -> {
                        routerState.getStyleClass().remove("highlight");
                        routerState.getStyleClass().add("warn");
                        duration.getStyleClass().remove("highlight");
                        spinner.setI2PIconId("not-connected");
                        spinner.stop();
                    }
                }
            }

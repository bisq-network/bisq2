...
@Slf4j
public class AdvancedCashFormView extends FormView<AdvancedCashFormModel, AdvancedCashFormController> {
    ...
    public AdvancedCashFormView(AdvancedCashFormModel model, AdvancedCashFormController controller) {
        ...
        Label selectedCurrenciesLabel = new Label(Res.get("paymentAccounts.advancedCash.selectedCurrencies"));
        selectedCurrenciesLabel.getStyleClass().add("bisq-text-1");

        selectedCurrenciesFlowPane = new FlowPane(5, 10);

        selectedCurrenciesErrorLabel = new Label(Res.get("paymentAccounts.createAccount.accountData.advancedCash.selectedCurrencies.error"));
        selectedCurrenciesErrorLabel.getStyleClass().add("material-text-field-error");

        VBox.setMargin(accountNr, new Insets(0, 0, 10, 0));
        content.getChildren().addAll(accountNr,
    ...
    protected void onViewAttached() {
        ...
        selectedCurrenciesErrorLabel.visibleProperty().bind(model.getCurrencyErrorVisible());
        selectedCurrenciesErrorLabel.managedProperty().bind(model.getCurrencyErrorVisible());
        ...
        selectedCurrenciesFlowPane.getChildren().addAll(getCurrencyEntries(model.getCurrencies(),
                model.getSelectedCurrencies()));
    }

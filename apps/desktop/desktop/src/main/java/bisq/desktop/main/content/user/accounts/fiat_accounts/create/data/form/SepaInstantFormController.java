...
@Slf4j
public class SepaInstantFormController extends FormController<SepaInstantFormView, SepaInstantFormModel, SepaInstantAccountPayload> {
    private Subscription ibanPin;

    public SepaInstantFormController(ServiceProvider serviceProvider) {
        super(serviceProvider);
    }

    @Override
    protected SepaInstantFormView createView() {
        return new SepaInstantFormView(model, this);
    }

    @Override
    protected SepaInstantFormModel createModel() {
        ...
        List<Country> allNonEuroCountries = FiatPaymentRailUtil.getSepaNonEuroCountries().stream()
                ...
                .sorted(Comparator.comparing(Country::getName))
                .collect(Collectors.toList());
        return new SepaInstantFormModel(StringUtils.createUid(),
                allSepaCountries,
                allEuroCountries,
                allNonEuroCountries);
    }

...
public class CashDepositFormController extends FormController<CashDepositFormView, CashDepositFormModel, CashDepositAccountPayload> {
    public CashDepositFormController(ServiceProvider serviceProvider) {
        super(serviceProvider);
    }

    @Override
    protected CashDepositFormView createView() {
        return new CashDepositFormView(model, this);
    }

    @Override
    protected CashDepositFormModel createModel() {
        ...
        return new CashDepositFormModel(StringUtils.createUid(),
                countries,
                currencies,
    ...
    public boolean validate() {
        ...
        boolean isRequirementsValid = model.getRequirementsValidator().validateAndGet();
        if (!isCurrencySet ||
                !isHolderNameValid ||
                !isHolderIdValid ||
                !isRequirementsValid) {
        ...
        if (BankAccountUtils.useValidation(countryCode)) {
            ...
            boolean isValid = isBankNameValid &&
                    isBankIdValid &&
                    isBranchIdValid &&
    ...
    @Override
    public CashDepositAccountPayload createAccountPayload() {
        return new CashDepositAccountPayload(model.getId(),
                ...
                Optional.ofNullable(model.getRequirements().get()));
    }
    ...
    void onSelectCountry(Country selectedCountry) {
        ...
        model.getRequirements().set(null);
    ...
    private void applySelectCountry(Country selectedCountry) {
        if (selectedCountry != null) {
            ...
            model.getUseValidation().set(true);
            ...
            model.getIsBankNameVisible().set(true);

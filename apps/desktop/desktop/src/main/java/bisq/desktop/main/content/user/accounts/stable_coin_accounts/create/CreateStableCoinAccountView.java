...
@Slf4j
public class CreateStableCoinAccountView extends NavigationView<VBox, CreateStableCoinAccountModel, CreateStableCoinAccountController> {
    ...
    public CreateStableCoinAccountView(CreateStableCoinAccountModel model, CreateStableCoinAccountController controller) {
        super(new VBox(), model, controller);

        root.setPrefWidth(OverlayModel.WIDTH);
        root.setPrefHeight(POPUP_HEIGHT);

        Label dataLabel = createAndGetProgressLabel(Res.get("user.stableCoinAccounts.create.headline"));
        progressLabelList.add(dataLabel);

        Label summaryLabel = createAndGetProgressLabel(Res.get("user.stableCoinAccounts.create.summary.headline"));
        progressLabelList.add(summaryLabel);

        HBox progressBox = new HBox(10, dataLabel, getHLine(), summaryLabel);
        ...
        progressItemsBox.getChildren().addAll(Spacer.fillHBox(), progressBox, Spacer.fillHBox(), closeButton);
        ...
        createAccountButton = new Button(Res.get("user.stableCoinAccounts.create.saveButton"));
        ...
        if (progressIndex < progressLabelList.size()) {
            ...
            if (delay) {
                ...
                progressLabelAnimationScheduler = UIScheduler.run(
                                () -> progressLabelAnimation = Transitions.fade(label, OPACITY, 1, ManagedDuration.getHalfOfDefaultDurationMillis()))

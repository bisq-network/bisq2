...
@Slf4j
public class PayIdFormView extends FormView<PayIdFormModel, PayIdFormController> {
    private final MaterialTextField holderName;
    private final MaterialTextField payId;
    private Subscription runValidationPin;

    public PayIdFormView(PayIdFormModel model, PayIdFormController controller) {
        ...
        payId = new MaterialTextField(Res.get("paymentAccounts.payId.payId"),
                Res.get("paymentAccounts.payId.prompt"));
        payId.setValidators(model.getPayIdValidator());
        payId.setMaxWidth(Double.MAX_VALUE);

        content.getChildren().addAll(holderName, payId);
    }
    ...
    protected void onViewAttached() {
        ...
        if (StringUtils.isNotEmpty(model.getPayId().get())) {
            payId.setText(model.getPayId().get());
            payId.validate();
        }

        holderName.textProperty().bindBidirectional(model.getHolderName());
        payId.textProperty().bindBidirectional(model.getPayId());
    ...
    protected void onViewDetached() {
        ...
        payId.resetValidation();

        holderName.textProperty().unbindBidirectional(model.getHolderName());
        payId.textProperty().unbindBidirectional(model.getPayId());

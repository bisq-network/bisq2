...
public class WiseAccountDetails extends FiatAccountDetails<WiseAccount> {
    public WiseAccountDetails(WiseAccount account, AccountTimestampService accountTimestampService) {
        super(account, accountTimestampService);
    }

    @Override
    protected void addDetails() {
        WiseAccountPayload accountPayload = account.getAccountPayload();
        ...
        String allCurrencies = Res.get("paymentAccounts.createAccount.accountData.wise.allWiseCurrencies");
        String selectedCurrencies = selectedCurrencyCodes.stream()
                .map(FiatCurrencyRepository::getDisplayNameAndCode)
                .sorted()
                .collect(Collectors.joining(", "));
        Label selectedCurrenciesLabel = addDescriptionAndValueWithCopyButton(Res.get("paymentAccounts.wise.selectedCurrencies"),

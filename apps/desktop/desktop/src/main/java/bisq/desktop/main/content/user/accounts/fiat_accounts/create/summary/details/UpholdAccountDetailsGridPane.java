...
public class UpholdAccountDetailsGridPane extends FiatAccountDetailsGridPane<UpholdAccountPayload> {
    public UpholdAccountDetailsGridPane(UpholdAccountPayload accountPayload, FiatPaymentRail fiatPaymentRail) {
        super(accountPayload, fiatPaymentRail);
    }

    @Override
    protected void addDetails(UpholdAccountPayload accountPayload) {
        addDescriptionAndValue(Res.get("paymentAccounts.holderName"),
                accountPayload.getHolderName());
        addDescriptionAndValue(Res.get("paymentAccounts.uphold.accountId"),
        ...
        if (allCurrencyCodes.equals(selectedCurrencyCodes)) {
            selectedCurrencies = Res.get("paymentAccounts.createAccount.accountData.uphold.allUpholdCurrencies");
        } else {
        ...
        Label selectedCurrenciesLabel = addDescriptionAndValue(Res.get("paymentAccounts.uphold.selectedCurrencies"),
                selectedCurrencies);

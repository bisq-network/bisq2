...
public class WiseUsdAccountDetails extends FiatAccountDetails<WiseUsdAccount> {
    public WiseUsdAccountDetails(WiseUsdAccount account, AccountTimestampService accountTimestampService) {
        super(account, accountTimestampService);
    }

    @Override
    protected void addDetails() {
        WiseUsdAccountPayload accountPayload = account.getAccountPayload();
        ...
        Label addressLabel = addDescriptionAndValueWithCopyButton(Res.get("paymentAccounts.wiseUsd.beneficiaryAddress"),
                accountPayload.getBeneficiaryAddress());
        addressLabel.setTooltip(new BisqTooltip(accountPayload.getBeneficiaryAddress()));

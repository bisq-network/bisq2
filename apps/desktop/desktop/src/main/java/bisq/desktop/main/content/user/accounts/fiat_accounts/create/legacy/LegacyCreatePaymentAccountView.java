...
package bisq.desktop.main.content.user.accounts.fiat_accounts.create.legacy;
...
@Slf4j
public class LegacyCreatePaymentAccountView extends View<VBox, LegacyCreatePaymentAccountModel, LegacyCreatePaymentAccountController> {
    ...
    public LegacyCreatePaymentAccountView(LegacyCreatePaymentAccountModel model,
                                          LegacyCreatePaymentAccountController controller) {
        ...
        int width = 600;
        Label headlineLabel = new Label(Res.get("paymentAccounts.legacy.createAccount.headline"));
        headlineLabel.getStyleClass().add("bisq-text-headline-2");

        Label subtitleLabel = new Label(Res.get("paymentAccounts.legacy.createAccount.subtitle"));
        subtitleLabel.setTextAlignment(TextAlignment.CENTER);
        subtitleLabel.setMaxWidth(width);
        subtitleLabel.setMinHeight(40);
        subtitleLabel.getStyleClass().addAll("bisq-text-3", "wrap-text");

        accountName = new MaterialTextField(Res.get("paymentAccounts.legacy.createAccount.accountName"),
                Res.get("paymentAccounts.legacy.createAccount.accountName.prompt"));
        accountName.setPrefWidth(width);
        accountData = new MaterialTextArea(Res.get("paymentAccounts.legacy.accountData"),
                Res.get("paymentAccounts.legacy.createAccount.accountData.prompt"));
    ...
    protected void onViewDetached() {
        accountData.textProperty().unbindBidirectional(model.accountDataProperty());
        accountName.textProperty().unbindBidirectional(model.accountNameProperty());

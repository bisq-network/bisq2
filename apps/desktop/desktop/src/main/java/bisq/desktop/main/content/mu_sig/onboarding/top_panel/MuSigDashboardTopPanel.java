...
package bisq.desktop.main.content.mu_sig.onboarding.top_panel;
...
public class MuSigDashboardTopPanel {
    private final Controller controller;

    public MuSigDashboardTopPanel(ServiceProvider serviceProvider) {
      ...
      //  private final Label marketPriceLabel, marketCodeLabel, offersOnlineLabel, activeUsersLabel;
       // private final HBox marketPriceHBox;
        ...
        /*    Triple<Pair<HBox, VBox>, Label, Label> priceTriple = getPriceBox(Res.get("dashboard.marketPrice"));
            ...
            Pair<VBox, Label> reputationPair = getValueBox("Your reputation");
            reputationPair.getSecond().setText("15304");
            root.getChildren().add(reputationPair.getFirst());

            Pair<VBox, Label> limitPair = getValueBox("Trade limit as Buyer");
            limitPair.getSecond().setText("1500 USD");
            root.getChildren().add(limitPair.getFirst());

            Pair<VBox, Label> ratePair = getValueBox("Trade rate as Buyer");
            ratePair.getSecond().setText("3 trades / day");
            root.getChildren().add(ratePair.getFirst());
        }

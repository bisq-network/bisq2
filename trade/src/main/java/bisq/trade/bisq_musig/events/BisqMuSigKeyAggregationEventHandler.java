...
public class BisqMuSigKeyAggregationEventHandler extends TradeEventHandler<BisqMuSigTrade> {
    ...
    public BisqMuSigKeyAggregationEventHandler(ServiceProvider serviceProvider, BisqMuSigTrade trade) {
        super(serviceProvider, trade);
    }

    @Override
    public void handle(Event event) {
        if (!(event instanceof BisqMuSigKeyAggregationEvent keyEvent)) {
            return;
        }

        log.info("Performing MuSig key aggregation for trade {}", trade.getId());

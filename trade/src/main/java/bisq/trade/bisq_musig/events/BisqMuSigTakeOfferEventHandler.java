...
public class BisqMuSigTakeOfferEventHandler extends TradeEventHandler<BisqMuSigTrade> {
    ...
    public BisqMuSigTakeOfferEventHandler(ServiceProvider serviceProvider, BisqMuSigTrade trade) {
        super(serviceProvider, trade);
    }

    @Override
    public void handle(Event event) {
        if (!(event instanceof BisqMuSigTakeOfferEvent takeOfferEvent)) {
            return;
        }

        log.info("Processing take offer event for trade {}", trade.getId());

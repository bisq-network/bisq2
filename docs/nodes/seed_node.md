...
- A seed node operator need to make a DAO proposal for a bonded role (seed node). After acceptance by DAO voting you
  need to lock up the BSQ bond (10 000 BSQ)
- After the bond is locked, register your seed node in the Bisq2 `User Options/Nodes` screen. Follow the instructions
  there.
...
used as seed node operator should be republished before those 2 weeks are over, otherwise the operators username in the
table shows `N/A` and the profile ID is not available (which is required for verification). Seed node operators are
expected to run their application with the user profile used for registration before the profile gets removed from the
network._
...
As at least one seed node (and oracle node) is required for the registration process we need to use hard coded public
keys to allow those nodes to be used even there is no registration data available.

There will be only a minimum number of those nodes (2 seed nodes, 2 oracle nodes) and we refer to them as root nodes.
...
5. After you have started the seed node copy the onion address (and/or I2P) and add it to the Bisq 2 config files. There
   are several config files for the different applications (desktop, seed, oracle, rest api). Add your onion address to
   the `application.network.seedAddressByTransportType` entry and make a pull request.

> _Note: Seed nodes which are not included in the config are not used for first time users. Those seed nodes get
distributed by the oracle node and only after the first start the users will receive those.<br><br>

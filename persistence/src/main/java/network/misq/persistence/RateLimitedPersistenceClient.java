...
public abstract class RateLimitedPersistenceClient<T extends Serializable> implements PersistenceClient<T> {
    ...
    public RateLimitedPersistenceClient() {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            Thread.currentThread().setName("RateLimitedPersistenceClient-shutdownHook-" + getPersistence().getFileName());
            persistOnShutdown();
        }));
    }
    ...
    public CompletableFuture<Boolean> persist() {
        ...
        if (dropped) {
            ...
            dropped = false;
            return getPersistence().persistAsync(getCloneForPersistence()).whenComplete((r, t) -> {
    ...
    private void persistOnShutdown() {
        if (dropped) {
            dropped = false;
            getPersistence().persist(getCloneForPersistence());
        }
    }
}